...
class Tindakan extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_pasien, tindakan, obat, biaya', 'required'),
			array('id_pasien, obat, biaya', 'numerical', 'integerOnly'=>true),
			array('tindakan', 'length', 'max'=>1000),
			array('tanggal', 'length', 'max'=>20),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, id_pasien, tindakan, obat, biaya, tanggal', 'safe', 'on'=>'search'),
		);
	}
